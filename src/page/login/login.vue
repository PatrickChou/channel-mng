...
export default {
  ...
  methods: {
    //登录方法
    login(name) {
      this.validForm(name).then((valid) => {
        const param = Object.assign({}, this.userData)
        this.$store.dispatch('login', {param}).then(({status}) => {
          const redirect = decodeURIComponent(this.$route.query.redirect || '/')
          status === 200 && this.$router.push({path: redirect})
        })
      })
    }
  }
